...
1. DeWasaNode_Master.cpp (all features working, individual sensor error works on the front end)

## Not yet Tested Files (assume bugged or not working yet)
1. PenyemainNode.cpp
2. PeremajaanNode.cpp
...
1. **Edit the cpp files:**
   - Open the cpp file for the node you're configuring
...
1. **For Dewasa (Master) Node:**
   ...
   - Open `DeWasaNode_Master.cpp` in Arduino IDE
...
2. **For Penyemaian Node:**
   - Connect the ESP32 to your computer
   - Open `PenyemaianNode.cpp` in Arduino IDE
...
3. **For Peremajaan Node:**
   - Connect the ESP32 to your computer
   - Open `PeremajaanNode.cpp` in Arduino IDE
   - Select the correct board and port
   - Click Upload
   - Verify initialization in Serial Monitor
...
1. Make changes to the cpp files
...
The fuzzy logic system is implemented in the DeWasaNode_Master.cpp file. The controller:
