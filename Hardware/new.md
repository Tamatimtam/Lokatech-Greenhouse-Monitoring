...
The Dewasa node acts as the master, receiving combined data from the Peremajaan node, running a fuzzy logic controller based on average environmental conditions to manage actuators (simulated by LEDs), sending the aggregated system status to an MQTT server, and listening for manual override commands via MQTT.
...
*   **`MQTTManager/`**: Class handling WiFi connection and MQTT communication (payload generation, publishing sensor data, subscribing to control topic) for the Master node.
...
*   Update `ssid`, `password`, `mqtt_server`, `mqtt_port`, and `mqtt_topic` in `DeWasaNode_Master.cpp`.
    ...
    ```
    *   The Master node subscribes to `mqtt_control_topic` to receive commands.
...
```json
{
  ...
  "actuators": { // Current state and mode of actuators on the Master node
    ...
    "light": {
      "state": false,
      "mode": "manual" // Example: Light is manually turned off
    }
  }
}
```
...
*   The `actuators` object reflects the current state of the fan/light (as read from the pins) and whether they are being controlled by the fuzzy logic (`auto`) or by a user command (`manual`).
...
```
*   Receiving a `manual` command for a device will set its state directly and prevent the fuzzy logic controller from changing it until the mode is potentially reset back to `auto` (Note: automatic reset to `auto` is not currently implemented).
